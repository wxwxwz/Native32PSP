// Frozen pre-optimization decoder. Deliberately retain full-plane interpolation.
// Its default keeps the old color semantics; callers may explicitly opt into
// the residual-zero chroma correction at the final RGB conversion only.
#ifndef NATIVE32_TEST_REFERENCE_IMAGE_DECODER_H
#define NATIVE32_TEST_REFERENCE_IMAGE_DECODER_H
#include "core/image_decoder.h"
#include "core/endian.h"
#include "core/raster_limits.h"
#include <algorithm>

namespace n32_image_reference {
using namespace n32;
...
bool decodeImageYuv(const std::vector<u8>& data, RgbaImage* out,
                    bool neutralMissingChroma = false) {
    if (!out || data.size() < 8) {
        return false;
    }
    ...
    for (size_t y = 0; y < height; ++y) {
        for (size_t x = 0; x < width; ++x) {
            ...
            int c = (int)y22[li] - 16;
            u8 u = ci < u22.size() ? u22[ci] : 128;
            u8 v = ci < v22.size() ? v22[ci] : 128;
            if (neutralMissingChroma) {
                if (!u) u = 128;
                if (!v) v = 128;
            }
            int d = (int)u - 128;
